...
export function useGetCollectionData(collection_address: string | undefined ) {
    ...
    queryFn: async () => {
      try {
        ...
        const userMintBalance =
          account == null
            ? 0
            : await getUserMintBalanceNFT({ user_address: account.address, collection_address, mint_stage });
        const isMintEnabled = await getMintEnabledNFT({ collection_address });
